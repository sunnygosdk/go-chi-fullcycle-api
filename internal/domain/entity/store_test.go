...
func TestValidateNewStore(t *testing.T) {
	...
	assert.Equal(t, entity.ErrorStoreInvalidName, err, "ValidateNewStore should return an error")

	store, err = entity.NewStore("store", "")
	assert.Nil(t, store, "ValidateNewStore should return no entity")
	assert.NotNil(t, err, "ValidateNewStore should return an error")
	assert.Equal(t, entity.ErrorStoreInvalidAddress, err, "ValidateNewStore should return an error")

	store, err = entity.NewStore("s", "address")
	assert.Nil(t, store, "ValidateNewStore should return no entity")
	assert.NotNil(t, err, "ValidateNewStore should return an error")
	assert.Equal(t, entity.ErrorStoreMinLengthName, err, "ValidateNewStore should return an error")

	store, err = entity.NewStore("store", "a")
	assert.Nil(t, store, "ValidateNewStore should return no entity")
	assert.NotNil(t, err, "ValidateNewStore should return an error")
	assert.Equal(t, entity.ErrorStoreMinLengthAddress, err, "ValidateNewStore should return an error")
}
...
func TestValidateUpdateStore(t *testing.T) {
	...
	assert.Equal(t, entity.ErrorStoreAtLeastOneField, err, "ValidateUpdateStore should return an error")

	invalidName := ""
	err = store.Update(&invalidName, nil)
	assert.NotNil(t, err, "ValidateUpdateStore should return an error")
	assert.Equal(t, entity.ErrorStoreInvalidName, err, "ValidateUpdateStore should return an error")

	invalidAddress := ""
	err = store.Update(nil, &invalidAddress)
	assert.NotNil(t, err, "ValidateUpdateStore should return an error")
	assert.Equal(t, entity.ErrorStoreInvalidAddress, err, "ValidateUpdateStore should return an error")

	invalidName = "s"
	err = store.Update(&invalidName, nil)
	assert.NotNil(t, err, "ValidateUpdateStore should return an error")
	assert.Equal(t, entity.ErrorStoreMinLengthName, err, "ValidateUpdateStore should return an error")

	invalidAddress = "a"
	err = store.Update(nil, &invalidAddress)
	assert.NotNil(t, err, "ValidateUpdateStore should return an error")
	assert.Equal(t, entity.ErrorStoreMinLengthAddress, err, "ValidateUpdateStore should return an error")
}
...
func TestValidateDeleteStore(t *testing.T) {
	...
	assert.Equal(t, entity.ErrorStoreIsDeleted, err, "ValidateDeleteStore should return an error")
}
