...
func TestValidateUpdateProduct(t *testing.T) {
	...
	assert.Equal(t, entity.ErrorProductAtLeastOneField, err)
	...
	assert.Error(t, err, "Update should return an error")
	assert.Equal(t, entity.ErrorProductDescriptionMinLength, err)
...
func TestValidateDeleteProduct(t *testing.T) {
	...
	assert.Equal(t, entity.ErrorProductIsDeleted, err)
}
